...
impl Game {
    pub fn from_board(board: &str) -> Result<Game, BoardError> {
        ...
        fn check_and_set(pos: &mut Option<(usize, usize)>, x: usize, y: usize, err: BoardError) -> Result<(), BoardError> {
            match *pos {
                None => {
                    *pos = Some((x, y));
                    Ok(())
        ...
        for line in board.lines() {
            ...
            for c in line.chars() {
                match c {
                    ...
                    'M' => { 
                        check_and_set(&mut minotaur, col, line_number, BoardError::MultipleMinotaur)?; 
                        row.push(false); 
                    }
                    'G' => { 
                    ...
                    ' ' => { row.push(false); }
                    bad => { return Err(BoardError::InvalidCharacter(bad)); }
                }
                col += 1;
            }
            board_vec.push(row);
        ...
        if theseus == None { return Err(BoardError::NoTheseus); }
        if minotaur == None { return Err(BoardError::NoMinotaur); }
        if goal == None { return Err(BoardError::NoGoal); }
        Ok(Game {
    ...
    pub fn show(&self) {
        ...
        for y in 0..self.grid.board.len() {
            ...
            for x in 0..self.grid.board[y].len() {
                if (x, y) == self.minotaur { line.push(minotaur_char); continue; }
                if (x, y) == self.theseus { line.push(theseus_char); continue; }
                if (x, y) == self.goal { line.push(goal_char); continue; }
                if !self.grid.board[y][x] { line.push(' '); continue; }
                ...
                let down = y + 1 < self.grid.board.len() && x < self.grid.board[y + 1].len() && self.grid.board[y + 1][x];
    ...
    pub fn theseus_move(&mut self, command: Command) {
        if command == Command::Left && self.theseus.0 > 0 && 
        ...
        if command == Command::Up && self.theseus.1 > 0 && 
            self.theseus.0 < self.grid.board[self.theseus.1 - 1].len() &&
            !self.is_wall(self.theseus.1 - 1, self.theseus.0) {
                self.theseus.1 -= 1;
        }
        if command == Command::Down && self.theseus.1 < self.grid.board.len() &&
            self.theseus.0 < self.grid.board[self.theseus.1 + 1].len() &&
            !self.is_wall(self.theseus.1 + 1, self.theseus.0) {
                self.theseus.1 += 1;
        }
    ...
    pub fn status(&self) -> GameStatus {
        if self.theseus == self.goal { GameStatus::Win }
        else if self.theseus == self.minotaur { GameStatus::Lose }
